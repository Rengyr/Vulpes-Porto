    ...
    s.into_iter()
        .map(|time| -> Result<(u8, u8), <D as Deserializer>::Error> {
            ...
            let hours = time_split
                ...
                .and_then(|h| {
                    h.parse::<u8>()
                        .map_err(|_| D::Error::custom("can't parse hours"))
                });
            let minutes = time_split
                ...
                .and_then(|h| {
                    h.parse::<u8>()
                        .map_err(|_| D::Error::custom("can't parse minutes"))
                });
            match (hours, minutes) {
                (Ok(hours), Ok(minutes)) => {
                    if hours > 23 {
...
fn load_images(
    ...
    if removed > 0 {
        println!(
            "{}Removed {} images not found in json",
            SYSTEMD_NOTICE.get().unwrap_or(&"".to_string()),
            removed
        );
    }
    ...
    if removed_d > 0 {
        println!(
            "{}Removed from random deck {} images not found in json",
            SYSTEMD_NOTICE.get().unwrap_or(&"".to_string()),
            removed_d
        );
    }
...
fn post_image<'a>(
    ...
    let image_hash = match images_db.unused.is_empty() {
        ...
        false => images_db
            .unused
            .get(rng.gen_range(0..images_db.unused.len()))
            .unwrap()
            .to_owned(),
    };
    ...
    let response = client
        ...
        .header(
            "Authorization",
            "Bearer ".to_string() + app_config.token.to_string().as_str(),
        )
    ...
    let media_id: String = match media_json["id"].as_str() {
        ...
        None => {
            eprintln!(
                "{}Unable to get media id: {:?} for image {}",
                SYSTEMD_ERROR.get().unwrap_or(&"".to_string()),
                media_json,
                image.location
            );
...
fn save_images_ids(image_db: &mut ImageDB, app_config: &Config) {
    match File::create(app_config.not_used_images_log_location.clone()) {
        Ok(mut file) => {
            file.write_all(serde_json::to_string(&image_db).unwrap().as_bytes())
                .unwrap();
        }
        Err(e) => {
            eprintln!(
...
fn main() {
    ...
    let app_config = fs::read_to_string(&args[1]).unwrap_or_else(|_| {
        panic!(
            "{}Couldn't find config.json file",
            SYSTEMD_ERROR.get().unwrap_or(&"".to_string())
        )
    });

    let mut app_config: Config = serde_json::from_str(&app_config).unwrap_or_else(|_| {
        panic!(
    ...
    loop {
            ...
            || (failed_to_post && (Instant::now() - failed_to_post_time).as_secs() > 60 * 10)
        {
            ...
            if let Ok(image) = image {
                ...
                println!(
                    "{}Image text: {}",
                    SYSTEMD_INFO.get().unwrap_or(&"".to_string()),
                    image.msg.to_owned().unwrap_or_default()
                );
                println!(
                    "{}Image alt text: {}",
                    SYSTEMD_INFO.get().unwrap_or(&"".to_string()),
                    image.alt.to_owned().unwrap_or_default()
                );
                println!(
                    "{}{}/{} images left",
                    SYSTEMD_NOTICE.get().unwrap_or(&"".to_string()),
